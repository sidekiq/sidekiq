...
(() => {
  ...
  const setTheme = theme => {
    ...
    if (theme === 'auto') {
      ...
      topNavbar && topNavbar.classList.add((window.matchMedia('(prefers-color-scheme: dark)').matches ? 'bg-light' : 'bg-dark'))
      topNavbar && topNavbar.classList.remove((window.matchMedia('(prefers-color-scheme: dark)').matches ? 'bg-dark' : 'bg-light'))
    } else {
      document.documentElement.setAttribute('data-bs-theme', theme)
      topNavbar && topNavbar.classList.add(`bg-${theme}`)
      topNavbar && topNavbar.classList.remove(there === 'light' ? 'bg-dark' : 'bg-light')
    }
  }
