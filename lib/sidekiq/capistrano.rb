Capistrano::Configuration.instance.load do
  ...
  namespace :sidekiq do
    ...
    task :start, :roles => lambda { fetch(:sidekiq_role) } do
      ...
      run "cd #{current_path} && bundle exec sidekiq -e #{rails_env} -C #{current_path}/config/sidekiq.yml -d -P #{current_path}/tmp/pids/sidekiq.pid -l #{current_path}/log/sidekiq.log"
    end
