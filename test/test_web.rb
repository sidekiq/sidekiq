...
describe Sidekiq::Web do
  ...
  describe "busy" do
    it "can display workers" do
      Sidekiq.redis do |conn|
        conn.call("INCR", "busy")
        conn.call("SADD", "processes", "foo:1234")
        conn.call("HMSET", "foo:1234", "info", Sidekiq.dump_json("hostname" => "foo", "started_at" => Time.now.to_f, "queues" => [], "concurrency" => 10), "at", Time.now.to_f, "busy", 4)
        identity = "foo:1234:work"
        hash = {queue: "critical", payload: {"class" => WebWorker.name, "args" => [1, "abc"]}, run_at: Time.now.to_i}
        conn.call("HMSET", identity, 1001, Sidekiq.dump_json(hash))
      end
    ...
    it "can quiet a process" do
      ...
      assert_nil Sidekiq.redis { |c| c.call "LPOP", signals_key }
      post "/busy", "quiet" => "1", "identity" => identity
      assert_equal 302, last_response.status
      assert_equal "TSTP", Sidekiq.redis { |c| c.call "LPOP", signals_key }
    end
    ...
    it "can stop a process" do
      ...
      assert_equal 302, last_response.status
      assert_equal "TERM", Sidekiq.redis { |c| c.call "LPOP", signals_key }
    end
  end
  ...
  it "can sort on enqueued_at column" do
    Sidekiq.redis do |conn|
      (1000..1005).each do |i|
        conn.call("LPUSH", "queue:default", Sidekiq.dump_json(args: [i], enqueued_at: Time.now.to_i + i))
      end
    end
  ...
  it "can delete a queue" do
    Sidekiq.redis do |conn|
      conn.call("RPUSH", "queue:foo", "{\"args\":[],\"enqueued_at\":1567894960}")
    ...
    Sidekiq.redis do |conn|
      ...
      refute conn.call("EXISTS", "queue:foo") > 0
    end
  end
  ...
  it "can delete a job" do
    Sidekiq.redis do |conn|
      conn.call("RPUSH", "queue:foo", '{"args":[],"enqueued_at":1567894960}')
      conn.call("RPUSH", "queue:foo", '{"foo":"bar","args":[],"enqueued_at":1567894960}')
      conn.call("RPUSH", "queue:foo", '{"foo2":"bar2","args":[],"enqueued_at":1567894960}')
    end
    ...
    Sidekiq.redis do |conn|
      refute conn.call("LRANGE", "queue:foo", 0, -1).include?("{\"foo\":\"bar\"}")
    end
  end
  ...
  it "can delete scheduled" do
    params = add_scheduled
    Sidekiq.redis do |conn|
      assert_equal 1, conn.call("ZCARD", "schedule")
      post "/scheduled", "key" => [job_params(*params)], "delete" => "Delete"
      assert_equal 302, last_response.status
      assert_equal "http://example.org/scheduled", last_response.header["Location"]
      assert_equal 0, conn.call("ZCARD", "schedule")
    end
  end
  ...
  it "escape job args and error messages" do
    ...
    Sidekiq.redis do |conn|
      pro = "foo:1234"
      conn.call("SADD", "processes", pro)
      conn.call("HMSET", pro, "info", Sidekiq.dump_json("started_at" => Time.now.to_f, "labels" => ["frumduz"], "queues" => [], "concurrency" => 10), "busy", 1, "beat", Time.now.to_f)
      identity = "#{pro}:work"
      hash = {queue: "critical", payload: {"class" => "FailWorker", "args" => ["<a>hello</a>"]}, run_at: Time.now.to_i}
      conn.call("HMSET", identity, 100001, Sidekiq.dump_json(hash))
      conn.call("INCR", "busy")
    end
  ...
  def add_scheduled
    ...
    msg = {"class" => "HardWorker",
           ...
           "tags" => ["tag1", "tag2"]}
    Sidekiq.redis do |conn|
      conn.call("ZADD", "schedule", score, Sidekiq.dump_json(msg))
    end
    [msg, score]
  end
  ...
  def add_retry
    ...
    score = Time.now.to_f
    Sidekiq.redis do |conn|
      conn.call("ZADD", "retry", score, Sidekiq.dump_json(msg))
    end
  ...
  def add_dead(jid = SecureRandom.hex(12))
    ...
    score = Time.now.to_f
    Sidekiq.redis do |conn|
      conn.call("ZADD", "dead", score, Sidekiq.dump_json(msg))
    end
    [msg, score]
  end
  ...
  def add_worker
    ...
    Sidekiq.redis do |conn|
      conn.multi do |transaction|
        ...
        transaction.call("HMSET", key, "info", Sidekiq.dump_json("hostname" => "foo", "started_at" => Time.now.to_f, "queues" => []), "at", Time.now.to_f, "busy", 4)
        transaction.call("HMSET", "#{key}:work", Time.now.to_f, msg)
      end
    end
  end
